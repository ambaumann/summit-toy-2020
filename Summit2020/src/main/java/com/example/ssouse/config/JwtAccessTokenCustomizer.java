...
// https://docs.spring.io/spring-security-oauth2-boot/docs/current/reference/htmlsingle/#oauth2-boot-resource-server-jwt-single-key
...
public class JwtAccessTokenCustomizer extends DefaultAccessTokenConverter implements JwtAccessTokenConverterConfigurer {
  ...
  private List<GrantedAuthority> extractRoles(JsonNode jwt) {
    ...
    jwt.path(CLIENT_NAME_ELEMENT_IN_JWT)
      .fields()
      .forEachRemaining((field) -> field.getValue().path(ROLE_ELEMENT_IN_JWT)
        .elements()
        .forEachRemaining(r -> rolesWithPrefix.add("ROLE_" +field.getKey() + "_" + r.asText())));
